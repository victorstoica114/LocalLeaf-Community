...
Add `--create-project-test` to the builder command to exercise project creation instead of the two-client soak. The supplied existing project URL/name are verified to identify the intended test server. This mode creates one separate project named `LocalLeaf create test <timestamp>`, checks its authenticated listing, downloads its initial `main.tex` into an empty test workspace, and verifies that a local edit uploads automatically. It leaves the new project available for inspection and records its name and URL in `result.json`. Existing project files are not modified.
