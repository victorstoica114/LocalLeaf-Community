...
- A complete Create Project editor page with project name, server, account controls, blank template preview, and optional local folder selection
- Inline validation, creation and download progress, success actions, and local setup recovery that resumes the same remote project
- Exact folder/server/project synchronization consent handoff when opening a newly created local copy
- Automatic three-way merging of independent `.tex`, `.bib`, `.md`, `.sty`, and `.cls` edits in a worker with bounded time and memory; overlapping changes remain available for review
- Persistent synchronization ancestors, hashes, remote identities, versions, and pending write/deletion/attachment journals

### Changed

- Recover synchronization from transport events and a throttled check when returning to VS Code, with increasing outage delays and no periodic healthy-session reconnect or reconciliation
- Recognize empty workspaces containing only Git and VS Code metadata and download their remote files without a Local/Remote choice
- Coalesce filesystem events, wait for generated-file writes to settle, and recognize atomic file replacement
- Keep individual file failures visible and deduplicate unchanged background file-choice and expired-session notifications

### Fixed

- Preserve project creation results across closing and reopening the editor page; repeated clicks and uncertain responses cannot automatically replay project creation
- Reject occupied, linked, nested-project, or symbolic-link creation destinations without replacing their files or configuration
- Preserve document subscriptions and incoming events while refreshing the remote tree; rejoin and reconcile after an outage
- Recover uncertain document writes and interrupted attachment replacement while preserving pending deletions, newer local saves, unsaved edits, and remote changes
- Revalidate identities and revisions after delayed conflict/link confirmations, while allowing unrelated files to continue synchronizing
- Honor directory-only ignore rules and recover missed moves and unchanged remote deletions
- Serialize settings writes, publish them atomically, and discard stale asynchronous UI and synchronization initialization results
- Reject inconsistent ranged downloads and stop requests or browser-login upgrades after disposal
- Package the text merge worker at its runtime path so clean installations can merge independent edits

### Maintenance and validation

- Remove unused helpers, obsolete synchronization scheduling, duplicate mappings, and unused website styles; avoid repeated hashing and unchanged state writes
- Preserve UTF-8 BOMs and handle very large unterminated LaTeX comment blocks
- Expand local API, UI, filesystem, persistence, lifecycle, merge, packaging, and two-client integration regressions
- Verify synchronization and graphical project creation against a dedicated Overleaf server using normal SecretStorage authentication
- Update dependency overrides and remove unused type stubs

[0.2.15]: https://github.com/victorstoica114/LocalLeaf-Community/compare/v0.2.10...v0.2.15
